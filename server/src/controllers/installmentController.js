...
export async function listByStudent(req, res) {
  ...
  const enriched = rows.map((row) => {
    let alert = 'upcoming';
    const dueStr = getLocalYYYYMMDD(row.due_date);
    if (row.status === 'paid' || Number(row.paid_amount) >= Number(row.installment_amount)) {
      alert = 'paid';
...
export async function listAlerts(req, res) {
  ...
  const mapped = rows.map((row) => {
    let badge = 'upcoming';
    const dueStr = getLocalYYYYMMDD(row.due_date);
    if (dueStr < today) badge = 'overdue';
    else if (dueStr === today) badge = 'due_today';
    else badge = 'upcoming';
    return { ...row, badge };
  });
  res.json(mapped);
  ...
  const reminders = rows
    .map((row) => {
      ...
      const msg =
        ...
        `Due amount: LKR ${remaining.toFixed(2)}. Due date: ${getLocalYYYYMMDD(row.due_date)}. Please pay on time.`;
      return {
