...
export async function reactivate(req, res) {
  ...
  if (existing.status !== 'dropout') {
    return res.status(400).json({ error: 'Student is not marked as dropout' });
  }
  ...
  try {
    ...
    await connection.rollback();
    console.error('student reactivate failed', error.message);
    return res.status(500).json({ error: 'Failed to reactivate student' });
  } finally {
    connection.release();
  }
}
