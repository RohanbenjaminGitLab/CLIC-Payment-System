            ...
            {rows.map((s) => (
              <tr key={s.id} className="border-t border-slate-100">
                ...
                <td className="space-x-2 px-4 py-3">
                  <button
                    type="button"
                    className="text-[#751c58] hover:text-[#5a1443] font-medium transition-colors"
                    onClick={() => {
                  ...
                  {s.status === 'dropout' ? (
                    <button
                      ...
                      onClick={async () => {
                        const res = await api(`/students/${s.id}/reactivate`, {
                          method: 'POST',
                        });
                        const data = await parseJson(res);
                        if (!res.ok) return toast.error(data?.error || 'Failed to reactivate');
                  ...
                  {canDelete && (
                    <button
                      type="button"
                      className="rounded-lg px-2 py-1 text-sm font-medium text-red-600 ring-1 ring-red-200 hover:bg-red-50 transition-colors"
